...
if [[ ! -d "${DEST}" ]]; then
    ...
    PATH="${DEST}/bin/${PLATFORM}:${PATH}" \
    perl "${ROOT}/install-tl" --profile "${ROOT}"/texlive.profile --repository "${ROOT}" -logfile "${ROOT}"/install-tl.log -no-verify-downloads -no-gui -force-platform "${PLATFORM}"
